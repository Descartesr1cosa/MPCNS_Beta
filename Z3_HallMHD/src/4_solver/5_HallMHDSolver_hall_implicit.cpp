...
void HallMHDSolver::ComputeRHShallFromCurrentBface_()
{
    ...
    // 4) 用当前 B_face + J_edge + pinv 缓存计算 Ehall_edge（inner）
    ComputeHallE_AtEdges_EnergyPreserving_(); // 只填 Ehall_xi/eta/zeta（线积分量）

    // 5) 对 Ehall_edge 做 BC + halo（保证 curl 计算 face RHS 时边界/接口正确）
    ApplyBC_EdgeHallEMF_(); //   edge 的物理边界/极点修补 + halo

    // 6) curl(Ehall_edge) -> RHShall_face
    AssembleFaceRHSHall_FromEdgeHallEMF_Curl_();
}

void HallMHDSolver::RHShallCallback_(void *ctx)
{
...
void HallMHDSolver::ZeroRHShall()
{
    ...
    for (int iblock = 0; iblock < fld_->num_blocks(); iblock++)
    {
        // B_xi
        {
            auto &RHS = fld_->field(fid_.fid_RHShall_Bface.xi, iblock);
        ...
        // B_eta
        {
            auto &RHS = fld_->field(fid_.fid_RHShall_Bface.eta, iblock);
            const Int3 &sub = RHS.get_lo();
            const Int3 &sup = RHS.get_hi();
            for (int i = sub.i; i < sup.i; i++)
                for (int j = sub.j; j < sup.j; j++)
        ...
        // B_zeta
        {
            auto &RHS = fld_->field(fid_.fid_RHShall_Bface.zeta, iblock);
            const Int3 &sub = RHS.get_lo();
            const Int3 &sup = RHS.get_hi();
            for (int i = sub.i; i < sup.i; i++)
                for (int j = sub.j; j < sup.j; j++)
...
void HallMHDSolver::AssembleFaceRHSHall_FromEdgeHallEMF_Curl_()
{
    ...
    {
        auto &RHS_xi = fld_->field(fid_.fid_RHShall_Bface.xi, iblk);
        auto &RHS_eta = fld_->field(fid_.fid_RHShall_Bface.eta, iblk);
        auto &RHS_zeta = fld_->field(fid_.fid_RHShall_Bface.zeta, iblk);
        auto &Exi = fld_->field(fid_.fid_Ehall.xi, iblk);
        auto &Eeta = fld_->field(fid_.fid_Ehall.eta, iblk);
        auto &Ezeta = fld_->field(fid_.fid_Ehall.zeta, iblk);
